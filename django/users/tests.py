...
class UserTestCase(TestCase):
    def setUp(self):
        '''
        A single user with Admin privileges is created and stored into the test database.
        '''
...
# [START Update Tests]
    ...
    def test_update_own_user_no_admin_change(self):
        u = User.objects.get(username='admin')
        self.client.login(username='admin', password='Pass4312')
        response = self.client.post('/users/{0}/update'.format(u.id), {'username': 'admin' ,'is_staff': True, 'first_name': 'PruebaUpd', 'last_name': 'PruebaUpd'}, follow=True)
        last_url = response.request['PATH_INFO']
        self.assertEqual(response.status_code, 200)
        self.assertEqual('/users/{0}/'.format(u.id), last_url)
    ...
    def test_update_own_user_admin_change(self):
        u = User.objects.get(username='admin')
        self.client.login(username='admin', password='Pass4312')
        response = self.client.post('/users/{0}/update'.format(u.id), {'username': 'admin' ,'is_staff': False, 'first_name': 'PruebaUpd', 'last_name': 'PruebaUpd'}, follow=True)
        last_url = response.request['PATH_INFO']
        self.assertEqual(response.status_code, 200)
        self.assertEqual('/users/{0}/'.format(u.id), last_url)
        self.assertEqual(u.is_staff, True)
